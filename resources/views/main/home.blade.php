    ...
    <div class="justify-center w-full">
            ...
            <div class="relative h-56 overflow-hidden rounded-lg md:h-96">
                ...
                @foreach ($movieCarousel as $index => $mc)
                    <div class="{{ $index === 0 ? 'block' : 'hidden' }} duration-700 ease-in-out" data-carousel-item="{{ $index === 0 ? 'active' : '' }}">
                        <img src="{{ asset($mc->image_path) }}" class="absolute block w-full -translate-x-1/2 -translate-y-1/2 top-1/2 left-1/2" alt="{{ $mc->title }}">
                    </div>
                @endforeach
            </div>
            
            <!-- Slider indicators loop -->
        ...
        <div class="movie-container">
            ...
            <div class="movies">
                ...
                <div class="movie">
                    <image src="" alt="{{$mn->title}}" class="movie-image"></image>
                    <h3 class="movie-title">{{$mn->title}}</h3>
                    <span class="movie-rating">{{$mn->rating}}</span>
                </div>
                @endforeach
            </div>
            {{ $movieNow->links() }}
        ...
        <div class="container">
            ...
            <div class="movies">
                ...
                <div class="movie">
                    <image src="" alt="{{$mu->title}}" class="movie-image"></image>
                    <h3 class="movie-title">{{$mu->title}}</h3>
                    <span class="movie-rating">{{$mu->rating}}</span>
                </div>
                @endforeach
            </div>
            {{ $movieUp->links() }}
